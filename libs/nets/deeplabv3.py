...
@slim.add_arg_scope
def bottleneck_hdc(inputs, depth, depth_bottleneck, stride, rate=1, multi_grid=(1, 2, 4),
...
def deeplabv3(inputs, num_classes, depth=50, aspp=True, reuse=None, is_training=True):
    ...
    Args:
      ...
      aspp: Whether to use ASPP module, if True, will use 4 blocks with multi_grid=(1,2,4), 
            if False, will use 7 blocks with multi_grid=(1,2,1).
    ...
    if aspp:
        ...
        multi_grid = (1, 2, 1)
    with tf.variable_scope('resnet{}'.format(depth), [inputs], reuse=reuse) as sc:
        end_points_collection = sc.name + '_end_points'
        with slim.arg_scope(resnet_arg_scope(weight_decay=args.weight_decay, batch_norm_decay=args.bn_weight_decay)):
            with slim.arg_scope([slim.conv2d, bottleneck, bottleneck_hdc], outputs_collections=end_points_collection):
                with slim.arg_scope([slim.batch_norm], is_training=is_training):
                    ...
                    with tf.variable_scope('block4', [net]) as sc:
                        ...
                        for i in range(3):
                            with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
                                net = bottleneck_hdc(net, depth=base_depth * 4, depth_bottleneck=base_depth,
                                                     stride=1, rate=2, multi_grid=multi_grid)
                        net = slim.utils.collect_named_outputs(end_points_collection, sc.name, net)
                    ...
                    net = slim.conv2d(net, num_classes, [1, 1], stride=1, activation_fn=None,
