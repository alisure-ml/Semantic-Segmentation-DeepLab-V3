...
def random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label=255):
    ...
    combined_pad = tf.image.pad_to_bounding_box(combined, 0, 0, tf.maximum(crop_h, image_shape[1]),
                                                tf.maximum(crop_w, image_shape[2]))

    combined_crop = tf.random_crop(tf.squeeze(combined_pad, [0]), [crop_h, crop_w, 4])
    img_crop = combined_crop[:, :, :image_shape[-1]]
    label_crop = combined_crop[:, :, image_shape[-1]:]
